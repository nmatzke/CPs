...
######################################
# Things we want to download $matK
######################################
IDs = xls$matK
...
######################################
# Things we want to download $rbcL
######################################
IDs = xls$rbcL
...
######################################
# Things we want to download $rps16
######################################
...
for (i in 1:length(list_of_fastas))
{
  ...
  # Search for gene=matK in the sequence headers
  TF1 = grepl(pattern="gene=rps16", x=seqnames)
  TF2 = grepl(pattern="note=ribosomal protein S16", x=seqnames)# alternative way to identify rps16
  TF3 = grepl(pattern="product=ribosomal protein S16", x=seqnames)
  TF4 = grepl(pattern="protein=ribosomal protein S16", x=seqnames)
